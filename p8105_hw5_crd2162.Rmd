...
In this problem, we will conduct a simulation to explore power in a one sample t-test.

First, I will write a function called `sim_power` to create a random sample from a normal distribution with an n of 30, a sigma of 5, and a mu of 0. This function also calculates the sample mean (`mu_hat`) and sample sigma (`sigma_hat`). The sample is then returned as a tibble.
...
Then, I will use map to conduct 5000 iterations of the function above. The results of the iterations are saved as a dataframe called `sim_results`.
...
I will then write a function called `sim_power_repeat` in which the same 5000 iterations can be done, except the mu can be changed. This function also maps a tidied t-test to the 5000 iterations. It is a test of the hypothesis of mu = 0 using alpha = 0.05. The outputs from the t-tests are saved as a dataframe called estimates which are then unnested by the function. The unnested results of the t-tests on the iterations in this function are returned as `df`.
...
Now we run the sim_power_repeat function using map. In the chunk below, the `sim_power_repeat` function is mapped to 7 different iterations of mu (mu = 0:6). The 7 different iterations are then combined used bind_rows and consolidated into a dataframe called `final_sim_results`. 
...
Finally, the dataframe is tidied and called `final_sim_clean`. This dataframe includes only necessary columns and to include a new variable that calculates whether the null hypothesis was accepted or rejected called `null_hyp`. This variable is then used to calculate `power` for each population mean. 
...
Now I will create a plot that shows the proportion of times the null was rejected (`power`) on the y axis and the true value of mu on the x axis. There is no linear pattern to the changes in power with changes in population mean in this graph. This seems to indicate that changes in power are due to random variation in the sampling.
...
Next, we'll make a plot showing the average estimate of mu_hat on the y axis and the true value of the population mean on the x axis. There is a strong linear pattern to this graph; as the population mean increases by one unit, the average estimate of mu hat increases by one unit.
...
Lastly, we'll create a plot that shows the average estimate of mu hat only in samples for which the null was rejected on the y axis and the true value of mu on the x axis. This plot looks identical to the second plot created above, despite the fact that it only includes sample means from in which the null hypothesis was rejected. This indicates that the average estimate of mu hat for samples in which the null hypothesis is rejected is approximately the same as the population mean. This is because we conducted a large random sampling simulation of over 5000 samples per population mean. With large numbers of sampling from the same underlying population, we expect the statistic to closely approximate the parameter. 
